									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																																															<label>药品名称 <input type="text" id="yaopinmingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	<label>药品类型</label>
																<select name="yaopinleixing" id="yaopinleixingSelect">
																																	<option value=" "> </option>
																																					...
																																					<option value="放射性药品">放射性药品</option>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="yaopinbianhaoTh" class="sorting" onclick="sort('yaopinbianhao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">药品编号</th>
																																																			<th id="yaopinmingchengTh" class="sorting" onclick="sort('yaopinmingcheng')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">药品名称</th>
																																																			<th id="yaopinleixingTh" class="sorting" onclick="sort('yaopinleixing')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">药品类型</th>
																																																			<th id="yaopinkucunTh" class="sorting" onclick="sort('yaopinkucun')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">药品库存</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">药品图片</th>
																																																																	<th id="shiyingzhengzhuangTh" class="sorting" onclick="sort('shiyingzhengzhuang')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">适应症状</th>
																																																			<th id="jinjizhengzhuangTh" class="sorting" onclick="sort('jinjizhengzhuang')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">禁忌症状</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">药品成分</th>
																																																																	
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">注意事项</th>
																																																																	<th id="yaopinxiangqingTh" class="sorting" onclick="sort('yaopinxiangqing')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">药品详情</th>
	...
	<script >
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var yaopinkucunCell  = document.createElement('td');

												 yaopinkucunCell.innerHTML = item.yaopinkucun;

                
													row.appendChild(yaopinkucunCell);
				
            				var yaopintupianCell  = document.createElement('td');

								                  var yaopintupianImg = document.createElement('img');
				  yaopintupianImg.setAttribute('src',item.yaopintupian);
				  yaopintupianImg.setAttribute('height','100px');
				  yaopintupianImg.setAttribute('width','100px');
				  yaopintupianCell.appendChild(yaopintupianImg);

				
													row.appendChild(yaopintupianCell);
				
            				var shiyingzhengzhuangCell  = document.createElement('td');

												 shiyingzhengzhuangCell.innerHTML = item.shiyingzhengzhuang;

                
													row.appendChild(shiyingzhengzhuangCell);
				
            				var jinjizhengzhuangCell  = document.createElement('td');

												 jinjizhengzhuangCell.innerHTML = item.jinjizhengzhuang;

                
													row.appendChild(jinjizhengzhuangCell);
				
            				var yaopinchengfenCell  = document.createElement('td');

												  if(item.yaopinchengfen != null && item.yaopinchengfen != "" && item.yaopinchengfen.length >= 11){
				  	yaopinchengfenCell.innerHTML = item.yaopinchengfen.substring(0, 10) + "..."; 
				  }else{
				  	yaopinchengfenCell.innerHTML = item.yaopinchengfen;
				  }

				
													row.appendChild(yaopinchengfenCell);
				  	...
				  	zhuyishixiangCell.innerHTML = item.zhuyishixiang;
            				...
            				var yaopinxiangqingCell  = document.createElement('td');
												  ...
												  //如果图文详情包含图片
				  if(item.yaopinxiangqing && item.yaopinxiangqing.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.yaopinxiangqing.indexOf('<img');
					var endIndex = item.yaopinxiangqing.indexOf('>');				
					yaopinxiangqingCell.innerHTML = item.yaopinxiangqing.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
