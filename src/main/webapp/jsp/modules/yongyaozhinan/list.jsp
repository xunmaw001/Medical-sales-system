										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																																															<label>药品名称 <input type="text" id="yaopinmingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	<label>药品类型 <input type="text" id="yaopinleixingSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="zhinanbiaotiTh" class="sorting" onclick="sort('zhinanbiaoti')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">指南标题</th>
																													...
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">不良反应</th>
																																																																	
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">用法用量</th>
																																																																	<th id="yongyaozhinanTh" class="sorting" onclick="sort('yongyaozhinan')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">用药指南</th>
	...
	<script >
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
							...
							var zhinanbiaotiCell  = document.createElement('td');

												 zhinanbiaotiCell.innerHTML = item.zhinanbiaoti;

                
													row.appendChild(zhinanbiaotiCell);
				  ...
				  fengmiantupianCell.appendChild(fengmiantupianImg);
            				...
            				var buliangfanyingCell  = document.createElement('td');
												  ...
												  if(item.buliangfanying != null && item.buliangfanying != "" && item.buliangfanying.length >= 11){
				  	buliangfanyingCell.innerHTML = item.buliangfanying.substring(0, 10) + "..."; 
				  }else{
				  	buliangfanyingCell.innerHTML = item.buliangfanying;
				  }

				
													row.appendChild(buliangfanyingCell);
				
            				var yongfayongliangCell  = document.createElement('td');

												  if(item.yongfayongliang != null && item.yongfayongliang != "" && item.yongfayongliang.length >= 11){
				  	yongfayongliangCell.innerHTML = item.yongfayongliang.substring(0, 10) + "..."; 
				  }else{
				  	yongfayongliangCell.innerHTML = item.yongfayongliang;
				  }

				
													row.appendChild(yongfayongliangCell);
				
            				var yongyaozhinanCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.yongyaozhinan && item.yongyaozhinan.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.yongyaozhinan.indexOf('<img');
					var endIndex = item.yongyaozhinan.indexOf('>');				
					yongyaozhinanCell.innerHTML = item.yongyaozhinan.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.yongyaozhinan != null && item.yongyaozhinan != "" && item.yongyaozhinan.length >= 11){
				  		yongyaozhinanCell.innerHTML = item.yongyaozhinan.substring(0, 10) + "..."; 
				  	}else{
				  		yongyaozhinanCell.innerHTML = item.yongyaozhinan;
				 	}
				  }

                
													row.appendChild(yongyaozhinanCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yongyaozhinan/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"yongyaozhinan");
